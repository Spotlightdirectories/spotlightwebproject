-- ===============================================================
-- Migration: discovery_search_rpcs
--
-- Fixes the systemic "fetch everything, filter in the browser"
-- pattern found across discover-results.js (searchProducts,
-- searchServices), vendor-product.js and vendor-service.js
-- (both "similar items" queries). All four previously downloaded
-- entire tables on every page view/search with no server-side
-- filtering at all.
--
-- Also bakes the sponsored/verified ranking (already correctly
-- built once in vendor-profile.js's loadSimilarBusinesses) directly
-- into the similar-items SQL, so ranking happens at the database
-- level rather than being duplicated in JavaScript.
-- ===============================================================

BEGIN;

-- ---------------------------------------------------------------
-- SEARCH PRODUCTS (real server-side filtering)
-- ---------------------------------------------------------------
...
RETURNS TABLE (
  ...
  vendor_average_rating numeric,
  vendor_reviews_count integer,
  vendor_is_sponsored boolean,
  vendor_slug text,
...
AS $$
  SELECT
    ...
    v.is_sponsored, v.slug, v.logo_url
...
-- ---------------------------------------------------------------
-- SEARCH SERVICES (real server-side filtering)
-- ---------------------------------------------------------------
...
-- ---------------------------------------------------------------
-- SIMILAR PRODUCTS (for vendor-product.html)
-- Replaces the unfiltered "fetch every product in the database"
-- pattern. Ranking (sponsored/verified/rating) baked in.
-- ---------------------------------------------------------------
...
AS $$
  SELECT
    ...
    v.name, v.verification_status, v.average_rating, v.reviews_count, v.is_sponsored
  FROM vendor_products p
  JOIN vendors v ON v.id = p.vendor_id
...
-- ---------------------------------------------------------------
-- SIMILAR SERVICES (for vendor-service.html)
-- ---------------------------------------------------------------
...
-- ---------------------------------------------------------------
-- SIMILAR BUSINESSES (for vendor-profile.html)
-- vendor-profile.js already filters this correctly server-side;
-- this moves the ranking (previously done client-side in JS) into
-- SQL too, for consistency with the three functions above.
-- ---------------------------------------------------------------
...
AS $$
  SELECT
    ...
    v.average_rating, v.reviews_count, v.verification_status, v.is_sponsored
  FROM vendors v
