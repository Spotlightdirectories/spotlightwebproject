...
## ACHIEVED — Block 5, item 43

- [x] 43. Harmonize partner program page + build marketing assets — DONE. Both partner-program.html and partner-legal.html rebuilt to match Spotlight's real brand (Inter, Font Awesome, theme.css tokens, real navbar/footer) instead of their previous separate font/color/icon system. Corrected two real inaccuracies found during audit: the "5%" override wording (now correctly describes 5% of the referred partner's commission, paid separately by Spotlight) and removed all free_vendor commission references (confirmed suspended). Fixed a genuine backend gap alongside this work: the ₦30,000 monthly bonus is now persisted as a real database row via an extended trigger, not just calculated live in a browser tab (migration: 20260712_persist_monthly_partner_bonus.sql). Built two genuinely new assets: a Brand Guide tab and an interactive Earning Calculator, both using only real, confirmed figures. Both pages built as separate files first, tested live through several rounds of feedback, then promoted to the real filenames. Old versions archived as partner-program-legacy.* and partner-legal-legacy.*. Note: partner-create-account.html still shares the old off-brand stylesheet (renamed to partner-legal-shared.css during promotion to avoid breaking it) and was not in scope for this item.
